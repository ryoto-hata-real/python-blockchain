...
class BlockChain(object):
  ...
  def add_transaction(self, sender_blockchain_address, recipent_blockchain_address, value, sender_public_key=None, signature=None):
    transaction = utils.sorted_dict_by_key({
      'sender_blockchain_address': sender_blockchain_address,
      'recipient_blockchain_address': recipent_blockchain_address,
      'value': float(value)
    })
  ...
  def create_transaction(self, sender_blockchain_address, recipient_blockchain_address, value, sender_public_key=None, signature=None):
  ...
  def verify_transaction_signature(self, sender_public_key, signature, transaction):
    ...
    verifying_key = VerifyingKey.from_string(
      bytes().fromhex(sender_public_key), curve=NIST256p, hashfunc=hashlib.sha256)
    verified_key = verifying_key.verify(signature_bytes, message)
    return verified_key

  def valid_proof(self,transactions,previous_hash,nonce,dificulty=MINING_DIFICULTY):
  ...
  def mining(self):
    ...
    self.add_transaction(sender_blockchain_address=MINING_SENDER, recipent_blockchain_address=self.blockchain_address,value=MINING_REWORD)
  ...
  def resolve_conflicts(self):
    ...
    if longest_chain:
      ...
      logger.info({'action': 'resolve_conflict', 'status': 'replaced'})
      return True

    logger.info({'message': 'resolve_conflict', 'status': 'not replaced'})
    return False
